...
[2026-06-28] Task: 新增多画板详情页公开净化案例 | Changes: 新增 `docs/showcases/MULTI_ARTBOARD_BATCH_EXPORT_CN.md`，用公开 demo-kit 假数据说明多画板 PSD 的画板组织、变量归属、字段驱动、导出命名和常见排查；README、Demo、Pages、案例库索引、开源检查清单和增长测试同步增加入口。 | Risk: Low. 仅新增公开案例文档和静态链接，不改变应用运行、PSD 导出、后台 API 或发布包逻辑。
[2026-06-28] Task: 补齐公开工作流 GIF 演示 | Changes: 新增 `docs/assets/fdesign-workflow-demo.gif`，用公开工作台截图抽帧说明 PSD 模板预览、图片变量替换、Excel 字段绑定和批量导出路径；README、Demo、GitHub Pages、Roadmap、开源检查清单与开源增长测试同步引用。 | Risk: Low. 仅新增公开演示资产和静态链接，不改变应用运行、PSD 导出、后台 API、发布包或仓库设置。
[2026-06-28] Task: 补齐 Photoshop 导出失败公开排障样例 | Changes: 在 `docs/TROUBLESHOOTING_CN.md` 增加公开安全的导出失败样例表，覆盖 `IDAT: incorrect data check`、Photoshop 静默吞任务、批量中途失败、PSD/PSB 保存失败和替换后偏移叠字；README、FAQ、GitHub Pages、Roadmap、开源检查清单和增长测试同步增加入口。 | Risk: Low. 仅新增公开排障文档和静态链接，不改变应用运行、PSD 导出、后台 API 或发布包逻辑。
[2026-06-28] Task: 补齐图片匹配和图层命名公开排障样例 | Changes: 在 `docs/TROUBLESHOOTING_CN.md` 增加图片未绑定、角度错位、商品位复用、图层变量未识别和字段错列的净化样例，并把 GitHub issue #15 接入 README、FAQ、GitHub Pages、Roadmap、开源检查清单和增长测试。 | Risk: Low. 仅新增公开排障文档和静态链接，不改变应用运行、PSD 导出、后台 API 或发布包逻辑。
[2026-06-28] Task: 新增中文上手入口承接国内社区流量 | Changes: 新增 `docs/START_HERE_CN.md`，把首次访问者按看演示、本地试跑、理解字段绑定、查看案例、排障反馈和参与改进分流；README、GitHub Pages、开源检查清单和增长测试同步增加入口。 | Risk: Low. 仅新增公开上手文档和静态链接，不改变应用运行、PSD 导出、后台 API 或发布包逻辑。
