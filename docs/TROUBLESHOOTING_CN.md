...
下面这些样例适合公开讨论。请使用虚构文件名、净化字段和文字描述，不要上传私有 PSD、真实商品图、账号信息、后台截图或授权不清素材。

| 现象 | 可能原因 | 优先检查项 | 可公开反馈信息 |
| --- | --- | --- | --- |
| Excel 已选中记录，但图片变量仍未绑定 | 图片路径列为空、路径不可访问、字段名和 PSD 变量名不一致 | 只保留一行 CSV 和一张图片；确认字段映射里 `imageVariables` 指向的列存在；手动打开图片路径 | 净化后的 CSV 表头、单行样例、字段映射片段、图片名样例，例如 `FDX1001-C10-front.png` |
| 正面图、侧面图或 45 度图被匹配错位 | 文件名里的角度标记不清晰，或色号/款号里也包含 `45` 这类数字 | 把图片列拆成 `front_image`、`side_image`、`angle45_image`；文件名统一使用 `front`、`side`、`angle45` | 变量名、图片文件名规则、被匹配到的实际变量，不要上传原图 |
| 多个商品位复用同一张图后裁切或参考线不一致 | 不同变量共用同一个 image key，但画布参考线、裁切框或商品位规则不同 | 为不同商品位拆分图片字段，或确认每个变量都有独立参考线；先只跑一个商品位 | 净化后的变量列表、商品位名称、参考线/裁切规则摘要 |
| PSD 里能看到图层，但 Fdesign 变量列表没有识别到 | 图层重名、隐藏在复杂组内、智能对象嵌套过深，或命名没有进入变量清单 | 重新上传模板；把变量层改成唯一命名；先用最小 PSD 教程确认变量识别规则 | 净化后的图层路径，例如 `Hero/{img:main}`；变量数量；是否重新上传过模板 |
| 文本字段空白或填到了错误列 | CSV 表头有空格、编码残留、字段被改名，或旧字段映射没有同步 | 对照 CSV 第一行和 `field-map.example.json`；复制一行最小数据单独验证；清理表头空格 | 净化后的表头、字段映射片段、单行样例和期望填充结果 |

推荐反馈格式：

```text
现象：图片未绑定 / 角度匹配错位 / 图层未识别 / 文本字段错列
最短复现：公开演示包 + 单行净化 CSV + 一张示例图片
净化后的 CSV 表头：
净化后的单行样例：
净化后的图片文件名样例：
净化后的 PSD 图层路径样例：
期望结果：
实际结果：
已排查：图片路径存在；后端 /health 正常；模板已重新上传
不能公开：私有 PSD、真实商品图、账号信息、token、后台截图、未净化字段
```

如果你愿意补充真实案例，请优先使用 [图片匹配和图层命名排障样例收集 issue](https://github.com/Kriswd/Fdesign/issues/15)。提交前把字段名、图片名、图层路径和截图都换成公开安全的假数据。
