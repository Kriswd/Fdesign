...
更具体的公开反馈格式可以先看 [图片匹配和图层命名净化样例](./TROUBLESHOOTING_CN.md#51-图片匹配和图层命名净化样例)。如果愿意补充案例，可以使用 [图片匹配和图层命名排障样例收集 issue](https://github.com/Kriswd/Fdesign/issues/15)。
...
更细的依赖、端口、健康检查、图片匹配和 Photoshop 导出排查步骤，可以看 [中文排障清单](./TROUBLESHOOTING_CN.md)，其中已经补了图片匹配、图层命名、`IDAT: incorrect data check`、Photoshop 静默吞任务、批量中途失败和 PSD/PSB 保存失败的净化样例。
