...
/**
 * 在图层树中按 layerId 应用 patch（递归）
 * @param {any[]} layerList - 图层列表
 * @param {string} layerId - 图层ID
 * @param {Record<string, any>} patch - 要合并的字段
 * @returns {any[]} 新的图层列表
 */
...
function Canvas({
  ...
  /**
   * 绑定容器 ref，并同步到 state（避免渲染期读取 ref.current）
   * @param {HTMLDivElement|null} node - DOM 节点
   */
  ...
  /**
   * 绑定画板 ref，并同步到 state（避免渲染期读取 ref.current）
   * @param {HTMLDivElement|null} node - DOM 节点
   */
  ...
  /**
   * 绑定导出根节点 ref，并同步到 state（避免渲染期读取 ref.current）
   * @param {HTMLDivElement|null} node - DOM 节点
   */
  ...
  const rootContainerEl = useMemo(() => {
    if (!containerEl) return null;
    return containerEl.querySelector('.react-transform-component');
  }, [containerEl]);

  useEffect(() => {
    if (initialLayers && initialLayers.length > 0) {
      setLayers(initialLayers);
      window.debugLayers = initialLayers; // 添加全局变量以便调试
    }
  }, [initialLayers]);
  ...
  return (
    <div
        ...
        style={{
          ...
          display: 'flex',
          flexDirection: 'column', // Ensure scrolling works for large content
          padding: '16px',
        }}
      >
        <TransformWrapper
          ...
          >
            ...
            >
              ...
              {!readOnly && (
                <>
                  <Selecto
                    container={rootContainerEl || containerEl}
                  ...
                  <Moveable
                    ...
                    onDrag={({ target, beforeTranslate }) => {
                      ...
                      const nextX = snapshot.x + beforeTranslate[0] / zoom;
                      const nextY = snapshot.y + beforeTranslate[1] / zoom;
                    ...
                    onDragGroup={({ events }) => {
                      setLayers((prev) => {
                        ...
                        events.forEach(({ target, beforeTranslate }) => {
                          ...
                          target.style.left = `${nextX}px`;
                          target.style.top = `${nextY}px`;
                          next = patchLayerById(next, layerId, { x: nextX, y: nextY });
                        });
                        return next;
                      });
                    }}
                    ...
                    onResize={({ target, width, height, drag }) => {
                      ...
                      const nextX = snapshot.x + (drag?.beforeTranslate?.[0] || 0) / zoom;
                      const nextY = snapshot.y + (drag?.beforeTranslate?.[1] || 0) / zoom;
                      ...
                      setLayers((prev) =>
                        patchLayerById(prev, layerId, { x: nextX, y: nextY, width, height }),
                      );
                    }}
                    onResizeEnd={({ target }) => {
                      const layerId = target?.getAttribute?.('data-layer-id');
