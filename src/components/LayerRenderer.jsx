...
/**
 * 兼容历史数据结构，将简化字段补齐为 textData
 * @param {any} layer - 图层数据
 * @returns {any} 规范化后的 textData
 */
...
/**
 * 判断图层是否被选中（兼容旧的 selectedLayerId 与新的 selectedLayerIds）
 * @param {string} layerId - 图层ID
 * @param {string[]|undefined} selectedLayerIds - 选中列表
 * @param {string|null|undefined} selectedLayerId - 单选ID
 * @returns {boolean}
 */
...
const LayerRenderer = memo(function LayerRenderer({
  ...
  const textContent = useMemo(() => {
    const next = getTextData(layer)?.content;
  ...
  const wrapperStyle = useMemo(() => {
    ...
    const widthCss =
      Number.isFinite(layer.width) && layer.width > 0 ? `${layer.width}px` : 'auto';
    const heightCss =
      Number.isFinite(layer.height) && layer.height > 0 ? `${layer.height}px` : 'auto';
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (current !== textContent) el.innerText = textContent;
    } catch (error) {
      try {
        console.error('[error] 同步文字编辑内容失败', { layerId: layer?.id, error });
      } catch (e) {
        void e;
      }
    }
  }, [isEditing, layer?.id, readOnly, textContent]);
  ...
  return (
    ...
    >
      {layer.type === 'text' && (
        ...
        >
          {isEditing && !readOnly ? null : textContent}
        </div>
      )}
      ...
      {layer.type === 'shape' && (
        <div
          className={`shape-layer ${showBoundingBox ? 'debug-bounds' : ''}`}
          style={{
            width: '100%',
            height: '100%',
